function cell_prop = FQ3_TS_detect_v2(img)
...
size_detect        = img.settings.TS_detect.size_detect;
status_only_in_nuc = img.settings.TS_detect.status_only_in_nuc;
th_min_TS_DAPI     = img.settings.TS_detect.th_min_TS_DAPI; 
N_max_TS_total     = img.settings.TS_detect.N_max_TS_total;
N_max_TS_cell      = img.settings.TS_detect.N_max_TS_cell;

%- Parameters for offset considereation (TS)
dist_max_offset              = img.settings.TS_detect.dist_max_offset;
dist_max_offset_FISH_min_int = img.settings.TS_detect.dist_max_offset_FISH_min_int;

%=== Which image will be used for the detection?
img_det_type = img.settings.TS_detect.img_det_type; 
switch img_det_type
    
    case 'FISH_image'
        img_TS_detect = img.raw;
        img_2nd       = [];
...
%- With a very low threshold only one big connected component is found
if CC.NumObjects == 1 && length(CC.PixelIdxList{1}) >0.5*numel(img_xyz)
     warndlg('Only ONE large TS found. Define higher threshold!',mfilename); 
   disp('Only ONE large TS found. Define higher threshold!'); 
   
%- More than the specified total threshold was found
elseif CC.NumObjects > N_max_TS_total

   warndlg(['Too many TS found: ', num2str(CC.NumObjects),'. Define higher threshold.'],mfilename); 
   disp(['Too many TS found: ', num2str(CC.NumObjects)]); 
