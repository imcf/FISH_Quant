function pos_RNA = sim_TS_v1(sim_prop,cell_prop, cell_library_info)
...
%- How many RNAs in TS (foci)
n_RNA_in_foci  = pattern_prop.level.(pattern_level);

%- How many foci (TS) per cell 
n_foci_cell = 1;
...
%% Put TS into the nucleus

%== Get all possible positions to place foci within the nucleus
th_dist_membrane = 1.5*max(diameter);

ind_pos = find(dist_nucleus_3D<th_dist_membrane);  % Outside of nucleus and within distance of cell membrane
if isempty( ind_pos)
...
pos_RNA_TS = pos_RNA_foci_pix + repmat(pos_foci,n_RNA_in_foci,1)  ;   
