function stack = tiffread33(file_name, indices, varargin)

% tiffread, version 3.3 - April 16, 2014
%
% stack = tiffread;
...
%% Sub Function
    ...
    function pixels = read_pixels(offsets)
        if TIF.PlanarConfiguration == 2
            for c = 1:TIF.SamplesPerPixel
                bytes = read_plane(offsets(c), c, IMG.width*IMG.height);
                pixels{c} = reshape(bytes, IMG.width, IMG.height)';
            end
        else
            sp = TIF.SamplesPerPixel;
            npix = IMG.width * IMG.height;
            bytes = read_plane(offsets(1), 1, sp*npix);
            for c = 1:sp
                pixels{c} = reshape(bytes(c:sp:sp*npix+c-sp), IMG.width, IMG.height)';
            end
        end
    end
    ...
    function bytes = read_plane(offset, plane_nb, bytes_nb)
        ...
        if pos < bytes_nb
            warning('tiffread:','found fewer bytes than needed');
        end
    end
