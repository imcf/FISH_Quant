...
if exist(file_name,'file') == 2
    ...
    if par.use_tiffread
        ...
        %- Read image
        img_data = tiffread33(file_name);
        
        %- Convert to format used in FISH-quant
        img.NZ  = size(img_data,2);
        img.NX  = img_data(1).width;
        img.NY  = img_data(1).height;

        %- 3d Matrix with intensity values
        img.data = uint16(zeros(img.NY,img.NX,img.NZ ));
        for i=1:img.NZ
            img.data(:,:,i)=img_data(i).data;
        end

        %- Status update
        status_file = 1;
        
   %- More advanced option with bfread
    else
