...
for i = 1:length(ind_close_ch1)
    ...
    try
        ...
        name = [img_name_only,'___',cell_prop_ch1.label,'___CH1-ID_',num2str(ind_close_ch1(i)),'___CH2-ID_',num2str(ind_close_ch2(i))];
        saveas(gcf,fullfile(folder_img_indiv_coloc,name),'png');
        close(gcf);
    catch err
        disp(err)
    end
...
for i = 1:length(ind_plot)
   ...
   %- Find closest spots
    ...
    if flag.which_ch == 1                   
        name = [img_name_only,'___',cell_prop_ch1.label,'___CH1-ID_',num2str(ind_spot_loop),];
    elseif flag.which_ch == 2
        name = [img_name_only,'___',cell_prop_ch2.label,'___CH2-ID_',num2str(ind_spot_loop)];
    end

    saveas(gcf,fullfile(folder_img_indiv_not_coloc,name),'png');
    close(gcf);               
end