...
#FASTA Sequences: path
PathNameFasta <- '/Volumes/PILON_HD2/fmueller/Documents/Programming/R/Oligostan/Documentation/Tutorial_data/'
...
ScoreMin <- 0.9
#probes minimum length
TailleSondeMax <- 32
#probes maximum length
TailleSondeMin <- 26
#mimimum number of nucleotides between two probes (end to start positions)
DistanceMinInterSonde <- 2
#Use of Composition rules described by the PNAS article
PNASfilter <- TRUE
#Numbers of rules to be used (ex to use only rules 1,3 and 5 use c(1,3,5))
PNASfilterOption <- c(1,2,3,4,5)
...
MaskedFilter <- TRUE
#Percentage of masked nucleotides allowed by probes for the probe to be kept
MaxMaskedPercent <- 0.2
#Minimum number of probes by transcript for the transcrip to be kept in final probes list
minProbePerTranscrit <- 14
#Path to the REPEAT MASKER installed command
RepeatMaskerCommand <- '/usr/local/RepeatMasker/RepeatMasker '
####################################################END OF SETTINGS##################################################
